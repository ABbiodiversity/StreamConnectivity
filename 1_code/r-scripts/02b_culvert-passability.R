...
#define coefficients 
coefs c("SlopePoint", "Confluence",
  "Distance", "CMI", "Elevation")
...
# Create the confusion matrix for the first model
load("results/hanging-culvert-model/version-6/bootstrap/hanging-culvert-model_1.Rdata")
model.data$Prediction <- predict.gbm(brt.model, model.data,
                                     n.trees = brt.model$gbm.call$best.trees, type="response")

# Assessing if convert to favorability is reasonable
prevalence <- sum(brt.model$data$y) / length(brt.model$data$y)
model.data$Favorability <- plogis( qlogis(model.data$Prediction) - qlogis(prevalence))
...
save(brt.model, pass.threshold, confusion.matrix, file = "results/hanging-culvert-model/version-6/hanging-culvert-model-stats.Rdata")
...
load("results/hanging-culvert-model/version-6/bootstrap/hanging-culvert-model_1.Rdata")
load("results/hanging-culvert-model/version-6/hanging-culvert-model-stats.Rdata")
...
for(HUC in watershed.ids) {
        ...
        temp.node$NaturalRegion <- factor(temp.node$NaturalRegion)
        ...
        # Convert to favorability
        prevalence <- sum(brt.model$data$y) / length(brt.model$data$y)
        temp.node$Up <- plogis(qlogis(temp.node$Up) - qlogis(prevalence))
...
write_sf(culvert.gis, dsn = "results/hanging-culvert-model/version-6/provincial-hanging-culvert-model.shp")
...
boot.list <- list.files("results/hanging-culvert-model/version-6/bootstrap/", full.names = TRUE)
