# ---
# title: Network creation functions
# author: Brandon Allen
# created: 2025-01-11
# notes: Define functions that are required for generating the network that is used for analysis.
# ---


#' [Network Creation]
#'
#' [Creates two data frames that represent the edges or nodes of the network]
#'
#' @param [watershed.geodatabase] [Path for the geodatabases used for processing individual watersheds.]
#' @param [huc.scale] [Value defining the watershed scale used for analysis.]
#' @param [huc.unit] [Unique watershed ID.]
#' @param [hfi.year] [Year of HFI.]
#' @param [dam.layer] [Layer containing the location of dam infratructure in Alberta.]
#' @param [mineable.boundary] [Defines the boundary for the mineable region of Alberta.]
#' @param [Slope] [30m slope layer.]
#' @param [DEM] [30m DEM.]
#' @param [MAP] [Raster of potential mean annual precipitation (30 year climate normal).]
#' @param [Eref] [Raster of potential evapotranspiration (30 year climate normal).]
#' @param [NR] [Layer defining natural regions of Alberta.]
#' @param [AT.bridges] [Location of bridges provided by Alberta Transportation.]
#' @param [NP.rail.bridges] [Layer containing the culvert infrastructure present on rails in the mountain national parks.]
#' @param [NP.road.bridges] [Layer containing the culvert infrastructure present on roads in the mountain national parks.]
...
#' 
#' @return [Creates the edge and node data frames that are used for calculating connectivity.]
#' 

network_creation <- function(watershed.geodatabase, huc.scale, huc.unit, hfi.year, dam.layer, mineable.boundary,
                             Slope, DEM, MAP, Eref, NR, AT.bridges, NP.rail.bridges, NP.road.bridges, folder.name, arcpy) {
  
        # Generate the folder for storing results
        analysis.path <- paste0(getwd(), "/", folder.name, "/huc-", huc.scale, "/", hfi.year, "/connectivity/")
...
def autoIncrement():
    ...
    return rec"
        ...
        if (n.culverts == 0) {
                ...
                #################################
                # Natural Region Identification #
                #################################
                
                # Natural Subregion 
                arcpy$SpatialJoin_analysis(target_features = "Culverts",
                                           join_features = NR,
                                           out_feature_class = "NaturalRegion",
                                           join_operation = "JOIN_ONE_TO_MANY")
                ...
                #
                # Identify the natural region
                #
                
                naturalregion.df <- read_sf(dsn = watershed.geodatabase,
                                            layer = "NaturalRegion")
                
                stream.edges["NaturalRegion"] <- NA
                stream.edges$NaturalRegion <- naturalregion.df$NRNAME[match(stream.edges$TARGET_FID, naturalregion.df$InterID, nomatch = NA)]
                ...
                edge.data <- data.frame(TARGET_FID = stream.edges$TARGET_FID,
                                        ...
                                        NaturalRegion = stream.edges$NaturalRegion,
